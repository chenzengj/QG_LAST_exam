   ...
   @Override
    ...
    public boolean creatPost(Post post) {
        try{
            // JDBCUtils.initConnectionPool();
            //String sql = "INSERT INTO users (username, password,user_type) VALUES (?, ?,?)";
            int rowsAffected = JDBCUtils.executeUpdate("Insert INTO posts(title,content)VALUES(?,?)",
                    post.getTitle(),post.getContent()); // 执行插入操作
            if (rowsAffected > 0) {
                return true;
            } else {
                return false;
    ...
    public List<Comment> getComments(int postId) {
        ...
        try{
                ResultSet rst=JDBCUtils.executeQuery("select * from comments where post_id=?",postId);
                while (rst != null &&rst.next()){
